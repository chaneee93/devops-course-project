#!/usr/bin/env bash
# Keycloak 로컬 개발용 setup (M6-1 / M6-4)
# dev 모드는 재시작 시 데이터가 날아가므로, 이 스크립트로 realm/client/user를 재생성한다.
# ⚠️ admin/admin, test1234는 로컬 개발 전용 더미값. 운영/AWS에선 사용 안 함.
set -e
...
echo "▶ course realm 재생성 (자체 회원가입 허용)"
$KCADM delete realms/course 2>/dev/null || true
$KCADM create realms -s realm=course -s enabled=true -s registrationAllowed=true

echo "▶ client(course-frontend) 생성 (localhost + 127.0.0.1 허용)"
...
echo "✅ 완료 — 로그인: test@example.com / test1234, 회원가입 페이지 활성화됨"
